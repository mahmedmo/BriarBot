...
class TestRunner {
    ...
    async runFullWorkflowTest(heroName) {
        ...
        try {
            ...
            console.log('1️⃣ Character search...');
            ...
            console.log('3️⃣ Analyzing hero data...');
            ...
            if (!screenshot) {
                ...
                if (cached) {
                    ...
                    console.log(colorize('   ⚠️ Failed to cache image', 'yellow'));
    ...
    displayTestResults(results) {
        ...
        if (results.imageSize) {
            console.log(`   🖼️ Image Size: ${colorize(results.imageSize, 'green')}`);
        }
        ...
        if (results.duration) {
            console.log(`   ⏱️ Duration: ${colorize(results.duration + 'ms', 'cyan')}`);
        }
    ...
    async saveTestOutput(heroName, imageBuffer) {
        try {
            ...
            console.error(colorize(`   ⚠️ Failed to save test output: ${error.message}`, 'yellow'));
        }
    }
            ...
            this.rl.question(colorize('Select option (1-4): ', 'cyan'), async (choice) => {
                switch (choice) {
                    case '1':
                        ...
                        break;
                    case '2':
                        console.log(colorize('⚠️ This will delete ALL cached images!', 'red'));
